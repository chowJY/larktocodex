...
from .paths import BRIDGE_ROOT, PROJECTS_CONFIG_PATH, STATE_DIR
...
def normalize_workspace_root(value: Any) -> str:
    ...
    try:
        ...
        return os.path.normcase(str(Path(str(value)).expanduser().absolute()))
...
def read_projects_config(path: Path = PROJECTS_CONFIG_PATH) -> list[dict[str, Any]]:
    ...
    if isinstance(projects, dict):
        ...
        for workspace_root, project in projects.items():
            if not isinstance(project, dict):
                continue
            normalized = dict(project)
            if not configured_value(normalized.get("workspace_root")):
                normalized["workspace_root"] = workspace_root
            result.append(normalized)
        return result
    if not isinstance(projects, list):
        raise SystemExit(f"Invalid projects config: {path} projects must be a dict keyed by workspace_root")
...
def select_project_config(
    ...
    if workspace_root:
        target_workspace = normalize_workspace_root(workspace_root)
        for project in projects:
            if normalize_workspace_root(project.get("workspace_root")) == target_workspace:
                return project
        raise SystemExit(f"No project config found for workspace '{workspace_root}' in {PROJECTS_CONFIG_PATH}")
...
def load_config(
    ...
    project = select_project_config(project_name, workspace_root, use_first_project)
    if project is None:
        raise SystemExit(f"Missing project config in {PROJECTS_CONFIG_PATH}")
